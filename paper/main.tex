...
Subsection~\ref{subsec:constants}, the constants appearing in the blueprint
...
formalized mathematics, as explained in Subsection~\ref{subsec:constants}.
...
constants -- and indeed most constants written in earlier versions of the
...
Several minor mistakes in the blueprint were noticed during the
formalization: some proofs were slightly wrong, or the assumptions in some
lemmas had to be strengthened. When we had to strengthen the assumptions, the
stronger assumptions were always satisfied when the lemmas were used.
Therefore, all these issues were purely local and did not jeopardize the
global proof.

Let us describe quickly some of these minor issues.

\begin{itemize}
\item Lemma 11.1.6 is a version of the boundedness of the Hilbert transform
    on $L^2(\real)$. It is proved by localization and periodization,
    ultimately using that $\|S_N f\|_{L^2[0,2\pi]} \leq
...
\texttt{Real}, is that it encodes that the constant cannot be infinity and that it needs to be positive.
...
These tactics do not work for terms in $\ennreal$ or $\nnreal$.
